...
public class WorkingMessage {
    ...
    public void correctAttachmentState() {
        ...
        if(mSlideshow.hasOctstream()){
            ...
            mAttachmentType = SLIDESHOW;
            // The vCard can be considered a special slide only in the
            // case of only 1 slide in it, vCard can not as the elements
            // of slide which have many slides.
            if (slide.hasVcard()) {
                slide.removeVcard();
            }
        } else {
...
/*
        ...
        */
        ...
        clearConversation(mConversation, true, true);
    }
...
*/
    ...
    private String readDraftSmsMessage(Conversation conv) {
        ...
        if (haveDraft && conv.getMessageCount() == 0) {
            ...
            // the activity.
            clearConversation(conv, true, false);
        }
        if (Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
    ...
    public void clearConversation(final Conversation conv, boolean resetThreadId, boolean deleteDraft) {
        if(deleteDraft){
            asyncDeleteDraftSmsMessage(conv);
        }
